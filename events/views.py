...
class EventView(View):
    template_event = 'event.html'
        
    """This is responsible for detecting get method and redirecting """		
    def get(self, request, slug):
        event = Events.objects.filter(slug=slug).last()
        return render(request, self.template_event, { 'event':event })
    
    """This is responsible for detecting post method and processing information """
    def post(self, request):
        return render(request, self.template_event, { })
